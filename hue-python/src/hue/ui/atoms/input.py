...
from hue.types.core import BaseComponent, Component
from hue.utils import combine_classes, render_if

T_Autocomplete = Literal[
    "off",
    "on",
    "name",
    "email",
...
def _render_input(input: _BaseInput) -> Component:
    classes = combine_classes(
        _get_base_input_classes(
            disabled=input.disabled, aria_invalid=input.aria_invalid
        ),
        input.class_,
    )
    ...
    input_attrs = {}
    if isinstance(input, NumberInput):
        input_attrs = {
            "min": input.min,
            "max": input.max,
            "step": input.step,
...
@concatenate_classes
def _get_base_input_classes(disabled: bool, aria_invalid: bool) -> list[str]:
    """
    ...
    if aria_invalid:
        ...
        classes.extend(["border-surface-200", "dark:border-surface-100"])
