"""Curated showcases for the bring-your-own Icon component.
...
from hue_docs.showcase import builder, variant
...
# An example whose code is rendered verbatim, so preview and code stay in sync.
_variant = partial(variant, namespace=_NS)
...
def _setup(label: str, code: str, preview: str) -> Variant:
    ...
    return Variant(
        label=label,
        build=builder(textwrap.dedent(preview).strip(), _NS),
